  ...
  try {
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        console.log("*************Email sent:", info.response);
        res.status(200).json({ responseCode: 200, message: "User created successfully" });
      }
    });
  } catch (err) {
    next(err);
...
// logout function  - clear cookie and its sends a response but also it can handle other errors
//
...
// Proposed fix "Added an error handling block for the case where the password validation fails in the forgot_password function."
export const forgot_password = async function (req, res, next) {
  try {
    const user = await User.findOne({ email: req.body.email });
    if (!user) {
    ...
    const mailOptions = {
      ...
      to: user.email,
      subject: "Bookly Africa Password Reset",
      html: `<h2>Please click on the link below to reset your password</h2><p>${process.env.CLIENT_URL}/reset-password/${token}</p>`,
    };
...
// Proposed fix "Added an error handling block for the case where the password validation fails in the reset_password function."
